...
INPUT_DIRECTORY = "STOP_CORPUS"
INPUT_FOLDER = os.getcwd() + "/" + INPUT_DIRECTORY
N = len(os.listdir(INPUT_FOLDER))
TF_IDF_SCORE_STOPPED_LIST = "TF_IDF_SCORE_STOPPED_LIST.txt"
...
def getScoredDocs(query, tfidf, docWordCount):
    queryIndex = {}
    docScoreList = {}
    for doc in docWordCount.keys():
        docScoreList[doc] = 0
    for term in query:
        if term in tfidf:
...
def writeToFile(queries, tfidf, docWordCount):
    queryID = 1
    file = open(TF_IDF_SCORE_STOPPED_LIST, "w")       
    for query in queries:
        TFIDFScoreList = getScoredDocs(query, tfidf, docWordCount)
...
def main():
    ...
    writeToFile(queries, tfidf, docWordCount)
main()
